...
CI uses macOS 26 with Xcode 26.5. Commit and pull-request workflows run SwiftLint, `package/check_bundled_recipes`, `./action-build.sh package` (the documented `./action-install.sh && make` plus universal/package flags), and Periphery. After the package build they rerun the recipe check with `--require-installed`. The release workflow substitutes `./action-build.sh archive` so it also builds Sparkle's `sign_update` tool and the release archive.
