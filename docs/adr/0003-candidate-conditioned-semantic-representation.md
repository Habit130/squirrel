...
Retrieval evidence remains positive-only, bounded, and candidate-specific. A historical selection event can support only the candidate selected by that event; it never directly subtracts from another candidate. When no qualifying history exists, retrieval evidence is zero. Live **production** ranking remains at `alpha=0` and `gamma=0` until a candidate-conditioned representation passes the frozen quality, safety, and prospective gates. A later personal-experiment exception is recorded in [ADR-0004](0004-personal-experimental-first-use.md); it does not rewrite this production rule, #80's no-qualified-configuration outcome, or the parked #81–#84 chain.
...
- ANN issues #78 and #79 remain parked until the reset produces an exact shortlist. No prototype result enables live retrieval evidence automatically. Personal-experiment activation still requires later technical acceptance and habit's explicit enablement.
